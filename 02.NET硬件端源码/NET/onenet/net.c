...
unsigned char OneNet_FillBuf(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"ph\":%d,", Data_init.PH); 
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"mq_3\":%d", Data_init.MQ3); 
	strcat(buf, text);
...
unsigned char MQTT_FillBuf(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"ph_v\":\"%d\",", threshold_value_init.PH_value); 
	strcat(buf, text);
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"mq_3_v\":\"%d\",", threshold_value_init.MQ3_value); 
	strcat(buf, text);
